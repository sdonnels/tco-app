...
| Ivanti | Neurons for UEM, Endpoint Manager |
| JAMF | Jamf Pro |
| Kaseya | VSA |
| PDQ | PDQ Deploy, PDQ Connect |
| Recast Software | Right Click Tools, Endpoint Insights |
...
## FEATURE: RESTART ASSESSMENT (TCO Micro-Assessment Platform Page)

### Overview

Add the ability for a user to restart an in-progress assessment, clearing all current selections and returning to a clean initial state.

### UI Placement

- Add a **"Restart Assessment"** button in the **top navigation/toolbar** of the TCO Micro-Assessment Platform page.
- The button should be visually distinct but secondary (not the primary action) — use an outlined or ghost-style button so it doesn't compete with the main "Submit" or "Next" actions.
- Include a restart/refresh icon (e.g., circular arrow) alongside the label for quick recognition.

### Visibility & State Rules

- The Restart button should be **hidden or disabled by default** when the assessment is in its initial empty state.
- The button becomes **visible and enabled only after the user has made at least one selection** in any pillar/sub-pillar dropdown.
- If the user navigates back to the assessment page after previously making selections (e.g., via browser back or menu), the button should still reflect the current state — visible if selections exist, hidden if not.

### Confirmation Flow

When the user clicks "Restart Assessment":

1. Display a **modal confirmation dialog** with the following:
   - **Title:** "Restart Assessment?"
   - **Body text:** "This will clear all of your current selections and progress. This action cannot be undone."
   - **Primary action button:** "Restart" (destructive style — red or warning color)
   - **Secondary action button:** "Cancel" (default/neutral style)
2. If the user clicks **"Cancel"** → close the modal, no changes.
3. If the user clicks **"Restart"** → execute the reset (see below).

### Reset Behavior

When confirmed, the restart must:

- Clear all dropdown selections across every pillar and sub-pillar back to their default/placeholder state.
- Reset all scoring flags and risk indicators to their default (no flags).
- Clear any dependent/cascading dropdown state (e.g., if Vendor was selected, Platform and Version should also reset).
- Return the user to the top/beginning of the assessment form.
- The page should not reload — handle this as a client-side state reset.

### Edge Cases

- If the user has already submitted a completed assessment, the Restart button should start a **new** assessment, not modify the submitted one.
- If there is any auto-save or draft functionality, the restart should also clear the saved draft.
- Double-clicking the Restart button should not trigger multiple confirmation modals.
...
- [ ] Restart button is hidden/disabled when no selections have been made
- [ ] Restart button appears after at least one dropdown selection
- [ ] Confirmation modal displays with correct text, Restart (destructive) and Cancel buttons
- [ ] Cancel dismisses modal with no changes
- [ ] Restart clears all dropdowns, scoring flags, and cascading state
- [ ] After restart, user is returned to top of assessment
- [ ] Restart does not trigger a full page reload
- [ ] Double-clicking Restart does not open multiple modals
- [ ] Previously submitted assessments are not affected by restart
